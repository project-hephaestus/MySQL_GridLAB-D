...
windturb_dg::windturb_dg(MODULE *module)
{
	if (oclass==NULL)
	{
		...
		if (gl_publish_variable(oclass,
			...
			PT_KEYWORD,"GENERIC_SYNCH_NEW",(enumeration)GENERIC_SYNCH_NEW, PT_DESCRIPTION, "Generic model for a small, fixed pitch synchronous turbine",
			...
			PT_KEYWORD,"GENERIC_IND_NEW",(enumeration)GENERIC_IND_SMALL, PT_DESCRIPTION, "Generic model for a small induction, fixed pitch generator turbine",
			...
			PT_double, "flag_no_wind", PADDR(flag_no_wind), PT_DESCRIPTION, "flag_no_wind, 0 for not giving on market, 1 for giving",
...
int windturb_dg::init(OBJECT *parent)
{
	...
	switch (Turbine_Model)	{
		...
		case GENERIC_SYNCH_MID:	//Creates generic 100kW wind turbine, northwind 100
			...
			break;
		case GENERIC_IND_NEW:					
		case GENERIC_SYNCH_NEW:	//Creates generic 5 kW wind turbine, Fortis Montana 5 kW wind turbine
			blade_diam = 33.2;      // in m
			turbine_height = 40;   //in m
			q = 0;						//number of gearbox stages, no gear box
			Rated_VA = 390000;   //19000            // calculate from P & Q
			Max_P = 378000; //17800
			Max_Q = 130000; //5000
			Rated_V = 600;
			pf = 0.9;
			...
			Cp_rated = Cp_max-.05;	//	|
			ws_rated = 14.5;		//	|
			if (Turbine_Model == GENERIC_IND_NEW) {
				Gen_type = INDUCTION;
				Rst = 0.12;					
				Xst = 0.17;					
				Rr = 0.12;				
			...
			else if (Turbine_Model == GENERIC_SYNCH_NEW) {
				Gen_type = SYNCHRONOUS;
				Rs = 0.05;
				Xs = 0.200;
				Rg = 0.000;
...
TIMESTAMP windturb_dg::sync(TIMESTAMP t0, TIMESTAMP t1) 
{
	...
	//{
		...
		if (CP_Data == GENERAL_LARGE || CP_Data == GENERAL_MID || CP_Data == GENERAL_SMALL ||CP_Data == GENERAL_NEW )	
		{
			if (WSadj <= cut_in_ws)
			{	
				Cp = 0;	
...
//if(time_here+1==*time && t1!=prev_entry) //each 15 minutes
	if(t1!=prev_entry) //each minute
{
	 /////////////////////////////////
	...
	// int name_solar=stoi(OBJECTHDR(this)->name+16);

	 //string name(OBJECTHDR(this)->name);
	 //std::size_t pos = name.find("solar");
	 //std::string str3 = name.substr (pos);
	 //str3.erase(str3.begin()+0);
	 //int name_solar=stoi(str3);
	 
	  int name_solar=stoi(OBJECTHDR(this)->name+16);
	  printf("wind::::::%d",name_solar);
	 driver = get_driver_instance();
		...
		char Query[2300];
		sprintf(Query, "INSERT INTO energy_sources(timestamp,type,realPower,reactivePower,nodeID,price,powerOut) VALUES('%s','wind',%f,%f,%d,%f,'0')",timenext_run,TotalRealPow,TotalReacPow,name_solar,price);
