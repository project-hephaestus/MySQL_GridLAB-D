...
// Connection details
string server3   = "tcp://127.0.0.1:3306";
	...
	if(t1!=prev_entry) //each minute
{
	...
	// int name_solar=stoi(OBJECTHDR(this)->name+16);
	 ...
	 //int name_solar=stoi(str3);
	  ...
	  int name_solar=stoi(OBJECTHDR(this)->name+16);
	  printf("dielsel::::::%d",name_solar);
	 driver = get_driver_instance();
        connection = driver->connect(server3, username3, password3);
		...
		char Query[2300];
		sprintf(Query, "INSERT INTO energy_sources(timestamp,type,realPower,reactivePower,nodeID,price,powerOut) VALUES('%s','diesel',%f,%f,%d,%f,'0')",timenext_run,TotalPowerOutput.Re(),TotalPowerOutput.Im(),name_solar,price);
