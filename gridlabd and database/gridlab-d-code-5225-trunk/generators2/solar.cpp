	...
	if(t1!=prev_entry) //each minute
{
	...
	// int name_solar=stoi(OBJECTHDR(this)->name+16);
	 ...
	 driver = get_driver_instance();
		...
		char Query[2300];
		sprintf(Query, "INSERT INTO energy_sources(timestamp,type,realPower,reactivePower,nodeID,price,powerOut) VALUES('%s','solar',0,0,%d,%f,%f)",timenext_run,name_solar,price,VA_Out);
		printf("solar: %s",Query);		
