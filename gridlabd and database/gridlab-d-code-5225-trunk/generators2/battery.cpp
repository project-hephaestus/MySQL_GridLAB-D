...
static double take_from_wind=0;
...
battery::battery(MODULE *module)
{
	if (oclass==NULL)
	{
		...
		if (gl_publish_variable(oclass,
			...
			PT_int16, "give_now", PADDR(give_now),
			...
			PT_object,"triplex_meter_charged",PADDR(triplex_meter_charged),
...
{
	ofstream myfile("file.txt", std::ios_base::app | std::ios_base::out);
	myfile<<"OBJECTHDR(this)->id"<<"    "<<"clear_price"<<"    "<< "clear_quantity" <<"    "<<"total_power_form_batteries"<<"			"<< "seller_total "<<"    "<<"total_buyer"<<"    "<<"take_from_wind" <<"    "<<"solar_total"<<"    "<<"power_absolute_total"<<"    "<<"real_power"<<"    "<<"B_scheduled_power1"<<"    "<<"B_scheduled_power2"<<"\n";
	 myfile.close();
		////////////////////////////////////SEARCH////////////////////////////////////
	first_time=0;

	 static FINDLIST *xt2=NULL;
				 xt2=gl_find_objects(FL_NEW,FT_CLASS,SAME,"generator_controller",FT_END);
				 OBJECT *firstt2= gl_find_next(xt2,NULL);
				 OBJECT *it2;
				 for(it2=firstt2;it2!=NULL;it2=it2->next)
				 {
				
					 if(gl_object_isa(it2,"generator_controller"))
				     {

						 
						 if(OBJECTHDR(this)->id==it2->parent->id)
						{
							battery_contr_object=it2;
						}
						 else
						 {
							// battery_contr_object=NULL;
						 }
				...
				static FINDLIST *xt1=NULL;
				 ...
				 xt11=gl_find_objects(FL_NEW,FT_CLASS,SAME,"windturb_dg",FT_END);
				 OBJECT *firstt11= gl_find_next(xt11,NULL);
				 OBJECT *it11;
				 for(it11=firstt11;it11!=NULL;it11=it11->next)
				 {
				
					 if(gl_object_isa(it11,"solar"))
				     {
					  ...
					  if(gl_object_isa(it11,"windturb_dg"))
				     {
		...
		obj1 = gl_find_next(climates,NULL);

		if (gl_object_isa(obj1,"climate"))
		{
			climate_object=obj1;
		}
...
{
	char timestart[128];
 gl_printtime(t1, timestart, 127);

 double B_scheduled_power1=0;
	double B_scheduled_power2=0;
	double *seller_total=gl_get_double_by_name(auction_object,"current_market.seller_total_quantity");
	double *total_buyer=gl_get_double_by_name(auction_object,"current_market.buyer_total_quantity");
		
	 clear_price=gl_get_double_by_name(auction_object,"current_market.clearing_price");
    clear_quantity=gl_get_double_by_name(auction_object,"current_market.clearing_quantity");
	
 

 ofstream myfile("file.txt", std::ios_base::app | std::ios_base::out);

	int16 *clearing_finished=gl_get_int16_by_name(auction_object,"clearing_finished");
	if(first_time==0)
	{
		first_time=t1;
	...
	if(strcmp(timestart,"2000-08-02 16:30:00 PDT")==0 ||strcmp(timestart,"2000-08-03 16:30:00 PDT")==0)
	{
		//system("pause");
		if(give_now==0)
		{
...
if(give_now==1)
{
	//////////////////////////// SEARCH ALL THE BATTERIES//////////////////////////////////
	double total_power_form_batteries=0;
	static FINDLIST *xt11=NULL;
				 xt11=gl_find_objects(FL_NEW,FT_CLASS,SAME,"battery",FT_END);
				 OBJECT *firstt11= gl_find_next(xt11,NULL);
				 OBJECT *it11;
				 for(it11=firstt11;it11!=NULL;it11=it11->next)
				 {
				
					 if(gl_object_isa(it11,"battery"))// && it11!=OBJECTHDR(this))
				     {

						double *energy=gl_get_double_by_name(it11,"Energy");
					    double *new_price=gl_get_double_by_name(it11,"price");
						if(*clear_price>= *new_price && *clear_quantity >= Energy/1000 && Energy>= E_Max*0.2 )
						{
						 total_power_form_batteries= (total_power_form_batteries+ *energy)/4;
						}
					 }
	...
	///////////////////////////////// SEARCH ALL SOLARS/////////////////////////////////
				 	double solar_total=0;
				
				 static FINDLIST *xt111=NULL;
				 xt111=gl_find_objects(FL_NEW,FT_CLASS,SAME,"solar",FT_END);
				 OBJECT *firstt111= gl_find_next(xt111,NULL);
				 OBJECT *it111;
				 for(it111=firstt111;it111!=NULL;it111=it111->next)
				 {
				
					 if(gl_object_isa(it111,"solar") )
				     {

						complex *solar=gl_get_complex_by_name(it111,"VA_Out");
						solar_total=solar_total+(*solar).Re();
	...
	if(first_time==2)
	{
		if( battery_contr_object!=NULL)
		{
					 //system("pause");
								
						if(*clear_price>= price && *clear_quantity >= Energy/1000 && Energy>= E_Max*0.2 )  //battery is in the market; it must be discharged
						{					
					
							B_scheduled_power1=Energy/4;
					
						}
						//else
						//{
						//B_scheduled_power1=0;
		...
		////////////////////////charge-discharge the battery from the solar or wind//////////////////////////////
		double *real_power;
		double *price_of_charged;
	    double power_absolute_total=0;
		
		if(gl_object_isa(triplex_meter_charged,"solar"))
		{
			OBJECT *meter=gl_get_object("triplex_meter");
			real_power=gl_get_double_by_name(meter,"measured_real_power");
			price_of_charged=gl_get_double_by_name(triplex_meter_charged,"price");
			
			wind_or_solar=IMIN_SOLAR;
		
		}
		else if(gl_object_isa(triplex_meter_charged,"windturb_dg"))
		{
			OBJECT *meter=gl_get_object("meter_new");
		 	real_power=gl_get_double_by_name(triplex_meter_charged,"TotalRealPow");
			price_of_charged=gl_get_double_by_name(triplex_meter_charged,"price");
			wind_or_solar=IMIN_WIND;
		
		}
		real_wind=gl_get_double_by_name(wind_object,"TotalRealPow");
		
		
		power_absolute_total=(*real_wind+solar_total)/1000;
	      
			 clear_price=gl_get_double_by_name(auction_object,"current_market.clearing_price");
			 clear_quantity=gl_get_double_by_name(auction_object,"current_market.clearing_quantity");
		

			 if(*clear_price >=*price_of_charged && *clear_quantity >= power_absolute_total)
			 {
				
				 B_scheduled_power2=0;
				 // myfile <<OBJECTHDR(this)->name<<"	"<<B_scheduled_power2 <<"\n";
			
			 }
		
			 else if(*clear_price >= *price_of_charged && *clear_quantity <= power_absolute_total)
			{
						
					
					
					if(wind_or_solar==1 && (*clear_quantity*1000 -total_power_form_batteries) < solar_total)
					{
					   B_scheduled_power2= - ( solar_total - (*clear_quantity*1000 -total_power_form_batteries)); //charge from solar only the quantity that is not sold

					   if(battery_state==3)
						{
								if(fabs(B_scheduled_power2) >0)
								{
							

									
									if(strcmp(triplex_meter_charged->name,"trip_shad_solarC11")==0){
									 OBJECT* battery= gl_get_object("oh_853_7E_Batteryc205");

									 char x_position_string[1024];
							         sprintf(x_position_string, "%f", - ( solar_total - (*clear_quantity*1000 -total_power_form_batteries)));
		
									 gl_set_value_by_name(battery,"scheduled_power",x_position_string);
									 gl_set_value_by_name(battery,"give_now","1");
									
									}

									else if(strcmp(triplex_meter_charged->name,"trip_shad_solarC95")==0)
									{
									    OBJECT* battery= gl_get_object("oh_853_7E_Batteryc206");
					...
					else if (wind_or_solar==1 && (*clear_quantity*1000 -total_power_form_batteries) >= solar_total)  //take from wind if the solar is not give the full energy
					{
					
						B_scheduled_power2=0;
						
						
						take_from_wind=(*clear_quantity*1000 -total_power_form_batteries)- solar_total;
						
						
						
					}
					
					if(wind_or_solar==0) //eimai wind kai se authn thn periptwsh tha fortizw thn mpataria tou wind
					{
						if(take_from_wind > 0)
						{
							gl_set_value_by_name(wind_object,"flag_no_wind","1");
						}
						else 
						{
							gl_set_value_by_name(wind_object,"flag_no_wind","0");
							
						}
						
						if(take_from_wind > *real_wind)
						{
						
						take_from_wind=*real_wind;
						}
						B_scheduled_power2= -fabs(*real_power)+take_from_wind; ///charge battery of wind
						...
						if(battery_state==3)
						{
								if(fabs(B_scheduled_power2) >0)
								{
									 ...
									 OBJECT* battery= gl_get_object("oh_853_7E_Batteryc204");
									 char x_position_string[1024];
							         sprintf(x_position_string, "%f", -fabs(*real_power)+take_from_wind);
						...
						//printf("antonia %f %f %f	",B_scheduled_power2,*real_power,take_from_wind);
					   //   myfile <<OBJECTHDR(this)->name<<"	"<<B_scheduled_power2 <<"\n";
			 ...
			 else if(*clear_price < *price_of_charged)  //solar-wind are not in market, so the battery charges
			 {
				// system("pause");
				 ...
				 if(*real_power <= 0)
				 {
					 ...
					 B_scheduled_power2=(*real_power); //battery is charged.
					//   myfile <<OBJECTHDR(this)->name<<"	"<<B_scheduled_power2 <<"\n";
	...
	//	printf("%d %f %f %f %f %f %f %f %f %f %f\n\n",OBJECTHDR(this)->id,*clear_price, *clear_quantity ,total_power_form_batteries ,*seller_total,*total_buyer,take_from_wind,power_absolute_total,*real_power,B_scheduled_power1,B_scheduled_power2);
			myfile<<OBJECTHDR(this)->id<<"    "<<*clear_price<<"    "<< *clear_quantity <<"    "<<total_power_form_batteries<<"			"<< *seller_total <<"    "<<*total_buyer<<"    "<<take_from_wind<<"  "<<solar_total<<"    "<<power_absolute_total<<"    "<<*real_power<<"    "<<B_scheduled_power1<<"    "<<B_scheduled_power2<<"\n";

		B_scheduled_power=B_scheduled_power1+B_scheduled_power2;
			 
			
			 gl_set_value_by_name(auction_object,"clearing_finished","0");
		...
		else if (gen_mode_v == GM_CONSTANT_PQ)
		{
			if (number_of_phases_out == 3) //Three phase meter
			{
				...
				if (prev_state == -1) //discharge
				{
					Energy = Energy - (1 / base_efficiency) * power_transferred * (double)dt / 3600;
					if(dt!=0){
					//printf("%f %f	%f",Energy, Energy/((double)dt / 3600),(1 / base_efficiency) * power_transferred );
					}
					if (Energy < 0)
						Energy = 0;
				}
