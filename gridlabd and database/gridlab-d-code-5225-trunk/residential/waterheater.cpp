...
string server2   = "tcp://10.96.12.53:3306";
string username2 = "lamprini";
...
if(t0!=prev_entry) //every each minute
{
   ...
   double *bid_quantity=gl_get_double_by_name(controller_object,"bid_quantity");
        ...
        connection = driver->connect(server2, username2, password2);
        statement = connection->createStatement();
        statement->execute("USE testing_db");
		char Query[230];
		sprintf(Query, "INSERT INTO devices(timestamp,deviceID,houseID,nodeID,phase,type,`load`,bidPrice,bidQuantity) VALUES('%s',%d,%d,%d,'%c','wh',%f,%f,%f)",timenext_run,hou_dev_ID,hou_dev_ID,nodeID,c_phase,actual_load,*bid_price,*bid_quantity);
	...
	if(t0!=prev_entry)//insert each moment
{
    ...
    //Here is the connection
		...
		sprintf(Query, "UPDATE devices SET `load`=%f ,bidPrice=%f, bidQuantity=%f WHERE timestamp='%s' and phase='%c' and houseID=%d and nodeID=%d and deviceID=%d and type='wh'",actual_load,*bid_price,*bid_quantity,timenext_run,c_phase,hou_dev_ID,nodeID,hou_dev_ID);
		printf("house: %s",Query);		
		statement->execute (Query);
	...
	prev_entry=t0;
	prev_entry=t1;//every 15minurtes;
