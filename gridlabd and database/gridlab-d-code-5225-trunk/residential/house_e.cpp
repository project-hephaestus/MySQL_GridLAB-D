...
{
	parent2=parent;
	///find the controller which i am its parent; to take the bidprice and bidquantity
	//controller_object;

	  static FINDLIST *x1=NULL;
  
    x1=gl_find_objects(FL_NEW,FT_CLASS,SAME,"controller",FT_END);
    OBJECT *first1= gl_find_next(x1,NULL);
    OBJECT *i1;

	 for(i1=first1;i1!=NULL;i1=i1->next)
     {
		 if(gl_object_isa(i1,"controller"))
		 {
			 printf("EDWWWWWWWW");
			 if(!strcmp(OBJECTHDR(this)->name,i1->parent->name))
			 {
			
			 controller_object=i1;
			 	 printf("HEREREEEEEE %s",controller_object->name);
...
{
	double *reactive_power=gl_get_double_by_name(parent2,"measured_reactive_power");
	OBJECT *obj = OBJECTHDR(this);
...
static FINDLIST *xt12=NULL;
	...
	char timebuf3[128];
	gl_printtime(t1,timebuf3,127);

	printf("%d	%d prev:%s	t0:%s",time_here,*time,timebuf2,timebuf3);
if(time_here+1==*time && t1!=prev_entry)	 //insert every 15 minutes
	//if(t0!=prev_entry)   //insert each moment
{
	string name(OBJECTHDR(this)->name);
	 
	////////////////////////////////////
	std::size_t pos = name.find("tm_");
	 std::string str3 = name.substr (pos);
	
	 std::size_t pos2 = str3.find("_");
	 std::string str4 = str3.substr (pos2);
	
	str4.erase(str4.begin()+0);
	char c_phase=str4.at(0); //phase
	str4.erase(str4.begin()+0);
	
	str4.erase(str4.begin()+0);
	std::size_t pos3 = str4.find("_");
	std::cout<<str4<<std::endl;
	std::string str44;
	str44.assign(str4);
	
	std::size_t found = str4.find("_");
	str4.erase(str4.begin()+found);
	
	int hou_dev_ID=stoi(str4);
	printf("GHGHJGHG:%d",hou_dev_ID);
	std::cout<<str4<<std::endl; //houseID AND devID

	std::string str5 = str44.substr (pos3);
	str5.erase(str5.begin()+0);
	std::cout<< str5; //nodeID
	int nodeID=stoi(str5);
	//const char *name2=str5.c_str();
	//OBJECT *node=gl_get_object((char *)name2);
	//int nodeID= node->id;
	//printf("gsfgfdgfdgdfg %d\n",nodeID);
    ...
    //Here is the connection
	 double *bid_price=gl_get_double_by_name(controller_object,"bid_price");
   double *bid_quantity=gl_get_double_by_name(controller_object,"bid_quantity");
   printf("NSDKFHDSFJDSHJSDF %s %f",controller_object->name,*bid_price);
		...
		char Query[2300];
		sprintf(Query, "INSERT INTO houses(timestamp,totalLoad,reactivePower,phase,houseID,nodeID,devID) VALUES('%s',%f,%f,'%c',%d,%d,%d)",timenext_run,total_load,*reactive_power,c_phase,hou_dev_ID,nodeID,hou_dev_ID);
		printf("house: %s",Query);		
		statement->execute (Query);
		sprintf(Query, "INSERT INTO devices(timestamp,deviceID,houseID,nodeID,phase,type,`load`,bidPrice,bidQuantity) VALUES('%s',%d,%d,%d,'%c','ac',%f,%f,%f)",timenext_run,hou_dev_ID,hou_dev_ID,nodeID,c_phase,hvac_load,*bid_price,*bid_quantity);
		printf("house: %s",Query);
		statement->execute (Query);
  ...
  //   if(t0!=prev_entry)  // insert each moment
{
    ...
    //Here is the connection
        ...
        connection = driver->connect(server, username, password);
        statement = connection->createStatement();
        statement->execute("USE testing_db");
		char Query[2300];
		sprintf(Query, "UPDATE houses SET totalLoad=%f ,reactivePower=%f WHERE timestamp='%s' and phase='%c' and houseID=%d and nodeID=%d and devID=%d",total_load,*reactive_power,timenext_run,c_phase,hou_dev_ID,nodeID,hou_dev_ID);
   ...
   double *bid_quantity=gl_get_double_by_name(controller_object,"bid_quantity");
		...
		//printf("house: %s",Query);		
		statement->execute (Query);
		sprintf(Query, "UPDATE devices SET `load`=%f ,bidPrice=%f, bidQuantity=%f WHERE timestamp='%s' and phase='%c' and houseID=%d and nodeID=%d and deviceID=%d and type='ac'",hvac_load,*bid_price,*bid_quantity,timenext_run,c_phase,hou_dev_ID,nodeID,hou_dev_ID);
		statement->execute (Query);
    delete statement;
    delete connection;
  ...
  //  system("pause");
	...
	prev_entry=t1;//each 15minutes
	//prev_entry=t0; //each minute
	return t2;
}
