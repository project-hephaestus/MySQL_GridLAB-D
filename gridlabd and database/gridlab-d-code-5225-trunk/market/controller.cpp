...
TIMESTAMP controller::sync(TIMESTAMP t0, TIMESTAMP t1){
	...
	int32 *clearat;
	  static FINDLIST *xt12=NULL;
				 ...
				 for(it12=firstt12;it12!=NULL;it12=it12->next)
				 {
				     ...
				     {
						clearat=gl_get_int32_by_name(it12,"clearing_at")  ;
						 
						 
				     }
			     }
	
	int32 time_here=gl_tominutes(t1);
	char timenext_run1[128];
	char timenext_run2[128];


	 gl_printtime(t1, timenext_run1, 127);
	  gl_printtime(prev_t1, timenext_run2, 127);
	   printf("antonia222222:    %s	%s",timenext_run1,timenext_run2);
								
	if(t1==prev_t1)
	{
	char timenext_run[128];
								 gl_printtime(t1, timenext_run, 127);
								 printf("antonia:    %d		%d   %s",time_here,*clearat,timenext_run);
								 system("pause");
	...
	printf("tewtsjgkfdghfdjghdfjlgfd\n");
	}
