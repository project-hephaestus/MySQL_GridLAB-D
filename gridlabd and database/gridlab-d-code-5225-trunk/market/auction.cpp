...
auction::auction(MODULE *module)
{
	if (oclass==NULL)
	{
		...
		if (gl_publish_variable(oclass,
			...
			PT_int32, "clearing_at", PADDR(clearing_at), PT_ACCESS, PA_REFERENCE, PT_DESCRIPTION, "g",
/**/		
...
TIMESTAMP auction::presync(TIMESTAMP t0, TIMESTAMP t1)
{
	...
	if (t1>=clearat)
	{
		...
		char name[64];
		clearat = nextclear();
		clearing_at=gl_tominutes(clearat);
		gl_printtime(clearat,timebuf,127);
		gl_printtime(t1,timebuf2,127);
		clearing_at=gl_tominutes(clearat);
		
		printf("in auction:%d %s %s",clearing_at,timebuf,timebuf2);
...
void auction::clear_market(TIMESTAMP t0)
{
	...
	int32 time_here=gl_tominutes(t0);
	///////////////////////////////////////////////////////////////////////////////////////////
	sql::Driver     *driver; // MySQL Driver Object
							...
							//Here is the connection
								...
								char timenext_run[128];
								 ...
								 //printf("%d		%d	%s",time_here,gl_tominutes(clearat),timenext_run);
								 //system("pause");

sprintf(Query, "INSERT INTO market_pool(timestamp,clearingPrice,clearingQuantity,sellerTotalQuantity,buyerTotalQuantity) VALUES ('%s',%f,%f,%f,%f)",timenext_run,current_frame.clearing_price,current_frame.clearing_quantity,current_frame.seller_total_quantity,current_frame.buyer_total_quantity);
