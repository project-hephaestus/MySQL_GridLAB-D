...
// Connection details
//string server   = "tcp://10.96.12.53:3306";
//string username = "lamprini";
...
node::node(MODULE *mod) : powerflow_object(mod)
{
	if(oclass == NULL)
	{
		...
		if(gl_publish_variable(oclass,
			...
			PT_object, "from_connected_to", PADDR(from_connected_to),PT_DESCRIPTION,"reference bfrom_connected_tous from which frequency is defined",
			...
			PT_int16, "serial_number",PADDR(serial_number), PT_DESCRIPTION, "serial_number",
...
int node::init(OBJECT *parent)
{
	...
	printf("init node  %s\n",OBJECTHDR(this)->name);
	serial_number=0;
	if(!gl_object_isa(obj,"triplex_meter"))
	{
	 char buffer3[128];
	  char buffer2[128];
   static FINDLIST *x1=NULL;
  
    x1=gl_find_objects(FL_NEW,FT_CLASS,SAME,"overhead_line",FT_END);
    OBJECT *first1= gl_find_next(x1,NULL);
    OBJECT *i1;
    ...
    for(i1=first1;i1!=NULL;i1=i1->next)
    {
     if(gl_object_isa(i1,"overhead_line") || gl_object_isa(i1,"underground_line") )// || gl_object_isa(i1,"switch") || gl_object_isa(i1,"regulator") || gl_object_isa(i1,"transformer"))
     {
     
     
	  gl_get_value_by_name(i1,"from",buffer2,sizeof(buffer2));

	  if(!strcmp(buffer2,obj->name))
      {
		  
		   gl_get_value_by_name(i1,"to",buffer3,sizeof(buffer3));
           from_connected_to= gl_get_object(buffer3); 
		   printf("from_%s ",from_connected_to->name);
		   string name_from(from_connected_to->name);
		   int name_f=stoi(name_from);
		  // sprintf(Query, "UPDATE nodes SET toNodeID = %d WHERE nodeID = %d",name_f,name_t);
...
TIMESTAMP node::presync(TIMESTAMP t0)
{
	...
	FUNCTIONADDR temp_funadd = NULL;
	 char timenext_run[128];
					 gl_printtime(t0, timenext_run, 127);
...
static FINDLIST *xt12=NULL;
				 ...
				 for(it12=firstt12;it12!=NULL;it12=it12->next)
				 {
				     ...
				     {
						 time=gl_get_int32_by_name(it12,"clearing_at")  ;
	...
	int time_here=gl_tominutes(t0);
	char timebuf2[128];
	gl_printtime(prev_entry,timebuf2,127);
	char timebuf3[128];
	gl_printtime(t0,timebuf3,127);

	//printf("node:presync %d	%d prev:%s	t0:%s",time_here,*time,timebuf2,timebuf3);
//	system("pause");
if(time_here+1==*time && t0!=prev_entry) //insert every 15 minutes
	//if(t0!=prev_entry)   //insert each moment
{	
	/////////////////////////////////////////////////////////////////
	...
	//INSERT INTO nodes(timestamp,nodeID,toNodeID,voltage_A_real,voltage_A_imag, voltage_B_real,voltage_B_imag,voltage_C_real,voltage_C_imag,current_out_A_real,current_out_A_imag,current_out_B_real,current_out_B_real,current_out_B_imag, current_out_C_real,current_out_C_imag,serial_number) VALUES (%d,%d,%d)",t0,x,from_connected_to_name,voltage[0].Re(),voltage[0].Im(),voltage[1].Re(),voltage[1].Im(),voltage[2].Re(),voltage[2].Im(),current[0].Re(), current[0].Im(),current[1].Re(), current[1].Im(),current[2].Re(), current[2].Im(),obj->id);
				...
				//Here is the connection
					...
					statement->execute("USE gridlabd");
					char Query[2500];
					
					
					string name_to(obj->name);
					 printf("post %s",obj->name);
					 int name_t=atoi(obj->name);

					 char timenext_run[128];
					 gl_printtime(t0, timenext_run, 127);
					 timenext_run[strlen(timenext_run)-4]='\0';
					if(from_connected_to!=NULL)
					{ 
					string name_from(from_connected_to->name);
					 name_f=stoi(name_from);
					 
					 
					sprintf(Query, "INSERT INTO nodes(timestamp,nodeID,toNodeID,voltage_A_imag,voltage_A_real,voltage_B_imag,voltage_B_real,voltage_C_imag,voltage_C_real,serial_number) VALUES ('%s',%d,%d,%f,%f,%f,%f,%f,%f,%d)",timenext_run,name_t,name_f,voltage[0].Re(),voltage[0].Im(),voltage[1].Re(),voltage[1].Im(),voltage[2].Re(),voltage[2].Im(),serial_number);
			//statement->execute (Query);			
				//sprintf(Query,"UPDATE nodes SET voltage_A_real=%f , voltage_A_imag=%f, voltage_B_real=%f ,voltage_B_imag=%f ,voltage_C_real=%f,voltage_C_imag=%f,current_out_A_real=%f,current_out_A_imag=%f,current_out_B_real=%f,current_out_B_real=%f,current_out_B_imag=%f, current_out_C_real=%f,current_out_C_imag=%f WHERE serial_number=%d and timestamp=%s and nodeID=%d and toNodeID=%d",voltage[0].Re(),voltage[0].Im(),voltage[1].Re(),voltage[1].Im(),voltage[2].Re(),voltage[2].Im(),current[0].Re(), current[0].Im(),current[1].Re(), current[1].Im(),current[2].Re(), current[2].Im(),serial_number-1,timenext_run,name_t,name_f);
						
					}
					 else 
					 {
						 sprintf(Query,"INSERT INTO nodes(timestamp,nodeID,toNodeID,voltage_A_imag,voltage_A_real,voltage_B_imag,voltage_B_real,voltage_C_imag,voltage_C_real,serial_number)  VALUES ('%s',%d,'0',%f,%f,%f,%f,%f,%f,%d)",timenext_run,name_t,voltage[0].Re(),voltage[0].Im(),voltage[1].Re(),voltage[1].Im(),voltage[2].Re(),voltage[2].Im(),serial_number);
						 //sprintf(Query,"INSERT INTO nodes(timestamp,nodeID) VALUES('%s',%d)",timenext_run,name_t);
						//sprintf(Query, "INSERT INTO nodes(timestamp,nodeID,voltage_A_imag,voltage_A_real,voltage_B_imag,voltage_B_real,voltage_C_imag,voltage_C_real,current_out_A_imag,current_out_A_real,current_out_B_imag,current_out_B_real,current_out_C_imag,current_out_C_real) VALUES VALUES ('%s',%d,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f)",timenext_run,name_t,voltage[0].Re(),voltage[0].Im(),voltage[1].Re(),voltage[1].Im(),voltage[2].Re(),voltage[2].Im(),current[0].Re(), current[0].Im(),current[1].Re(), current[1].Im(),current[2].Re(), current[2].Im());
						//statement->execute (Query);
						///	sprintf(Query,"UPDATE nodes SET voltage_A_real=%f , voltage_A_imag=%f, voltage_B_real=%f ,voltage_B_imag=%f ,voltage_C_real=%f,voltage_C_imag=%f,current_out_A_real=%f,current_out_A_imag=%f,current_out_B_real=%f,current_out_B_real=%f,current_out_B_imag=%f, current_out_C_real=%f,current_out_C_imag=%f WHERE serial_number=%d and timestamp=%s and nodeID=%d",voltage[0].Re(),voltage[0].Im(),voltage[1].Re(),voltage[1].Im(),voltage[2].Re(),voltage[2].Im(),current[0].Re(), current[0].Im(),current[1].Re(), current[1].Im(),current[2].Re(), current[2].Im(),serial_number-1,timenext_run,name_t);
...
TIMESTAMP node::postsync(TIMESTAMP t0)
{
	...
	printf("%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f\n\n",voltage[0].Re(),voltage[0].Im(),voltage[1].Re(),voltage[1].Im(),voltage[2].Re(),voltage[2].Im(),current[0].Re(), current[0].Im(),current[1].Re(), current[1].Im(),current[2].Re(), current[2].Im())	;			 
