...
string server2   = "tcp://127.0.0.1:3306";
...
triplex_meter::triplex_meter(MODULE *mod) : triplex_node(mod)
{
	...
	if (oclass==NULL)
	{
		...
		if (gl_publish_variable(oclass,
			...
			PT_int16, "central", PADDR(central),PT_DESCRIPTION,"central",
...
/*	if(central==1)
	{
			   ...
			   x1=gl_find_objects(FL_NEW,FT_CLASS,SAME,"overhead_line",FT_END);
				...
				{
				 if(gl_object_isa(i1,"transformer"))// || gl_object_isa(i1,"underground_line") )// || gl_object_isa(i1,"switch") || gl_object_isa(i1,"regulator") || gl_object_isa(i1,"transformer"))
				 {
     
     
				  gl_get_value_by_name(i1,"to",buffer2,sizeof(buffer2));

				  if(!strcmp(buffer2,obj->name))
				  {
		  
					   gl_get_value_by_name(i1,"from",buffer3,sizeof(buffer3));
	...
	//if(t0!=prev_entry)  // insert each moment
{
		if(central==1)
				...
				//	printf("%s sfdsfdsfsdf",OBJECTHDR(this)->name+5);
					 
					 int name_f=stoi(OBJECTHDR(this)->name+5);
					 char timenext_run[128];
					 gl_printtime(t0, timenext_run, 127);
					 timenext_run[strlen(timenext_run)-4]='\0';

					sprintf(Query, "INSERT INTO central_triplex_meter(timestamp,nodeID,realPower,reactivePower) VALUES ('%s',%d,%f,%f)",timenext_run,name_f,measured_real_power,measured_reactive_power);
					printf("%s",Query);
...
//		if (dt > 0 && last_t != dt)
	...
	char timebuf2[128];
	gl_printtime(prev_entry2,timebuf2,127);
	char timebuf3[128];
	gl_printtime(t1,timebuf3,127);

//	printf("post:triplex_meter:%d	%d prev:%s	t0:%s",time_here,*time,timebuf2,timebuf3);
	//system("pause");
if(time_here+1==*time && t1!=prev_entry2) //insert every 15 minutes
	//if(t1!=prev_entry2)   //insert each moment
{
		if(central==1)
				{
					    sql::Driver     *driver; // MySQL Driver Object
				...
				//Here is the connection
					...
					sprintf(Query, "UPDATE central_triplex_meter SET realPower=%f , reactivePower=%f where timestamp='%s' and nodeID=%d",measured_real_power,measured_reactive_power,timenext_run,name_f);
					printf("%s",Query);
					statement->execute (Query);
